...
export default function SuperAdminUsersPage () {
  ...
  const usersList = Array.isArray(searchedUsers)
    ? searchedUsers
    : pagedUsers?.page || []

  const selectedUser = useQuery(
    api.users.queries.index.getUserById,
    selectedUserId ? { userId: selectedUserId as unknown as Id<'users'> } : 'skip'
  )
                ...
                >
                  ...
                  <div className="col-span-4 flex items-center gap-2 text-xs">
                    <div className="truncate">
                      {[u.isAdmin && 'Admin', u.isStaff && 'Staff', u.isMerchant && 'Merchant']
                        .filter(Boolean)
                        .join(', ') || '—'}
                    </div>
          ...
          {selectedUser && (
            <div className="mt-6 rounded border p-4">
              ...
              <div className="grid gap-2 text-sm md:grid-cols-2">
                <div><span className="text-muted-foreground">Name:</span> {`${selectedUser.firstName || ''} ${selectedUser.lastName || ''}`.trim() || '—'}</div>
                <div><span className="text-muted-foreground">Email:</span> {selectedUser.email}</div>
                <div><span className="text-muted-foreground">Roles:</span> {[selectedUser.isAdmin && 'Admin', selectedUser.isStaff && 'Staff', selectedUser.isMerchant && 'Merchant'].filter(Boolean).join(', ') || '—'}</div>
                <div><span className="text-muted-foreground">Last login:</span> {selectedUser.lastLoginAt ? new Date(selectedUser.lastLoginAt).toLocaleString() : '—'}</div>
                <div><span className="text-muted-foreground">Total orders:</span> {selectedUser.totalOrders ?? 0}</div>
                <div><span className="text-muted-foreground">Total spent:</span> ${selectedUser.totalSpent?.toFixed?.(2) ?? '0.00'}</div>
              </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-1 text-xs">
                    {selectedUser.organizationMemberships.map((m) => (
                      <div key={`${m.organizationId}`}>/{m.organizationSlug} – {m.role} {m.isActive ? '' : '(inactive)'}
                      </div>
                    ))}
                  </div>
                </div>
