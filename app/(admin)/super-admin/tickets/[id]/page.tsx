...
export default function SuperAdminTicketDetailPage() {
  ...
  const updatesList = useMemo<TicketUpdate[]>(() => {
    if (!updates?.updates) return [];
    // Map backend data (which has 'update' field) to interface (which has 'status' field)
    return updates.updates.map((update: any) => {
      const { update: statusValue, ...rest } = update;
      return {
        ...rest,
            ...
            <CardContent className="p-4 pt-0 space-y-4">
              ...
              <div className="flex items-start gap-3">
                <Avatar className="h-8 w-8 mt-0.5">
                  {creatorImageUrl && <AvatarImage src={creatorImageUrl} alt={ticket.creatorInfo?.email || 'Unknown'} />}
