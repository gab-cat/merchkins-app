...
type TicketListItem = Doc<'tickets'>;
...
function CompactTicketCard({ ticket }: { ticket: TicketListItem }) {
  ...
  return (
    <Link href={`/super-admin/tickets/${ticket._id}`} className="block min-w-[280px] w-[280px]">
...
function CreateTicketDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Error handling is done in handleCreateTicket, but log for debugging
      console.error('Failed to create ticket:', error);
      // Don't clear form or close dialog on error - let user retry
    } finally {
...
export default function SuperAdminTicketsPage() {
  ...
  // My Assigned Tickets Query - only personal tickets assigned to current user
  const myAssignedTickets = useQuery(api.tickets.queries.index.getTickets, { assignedToMe: true, limit: 20, retryKey: assignedTicketsRetryTrigger });
  ...
  // Memoize baseArgs to prevent infinite loops
  ...
  const baseArgs = useMemo(
    () => ({
      ...
      personalOnly: true, // Filter for personal tickets (no organizationId) server-side
    }),
    [statusFilter, priority, debouncedSearch, dateRange.dateFrom, dateRange.dateTo]
  );

  // Query tickets and filter for personal tickets (no organizationId)
  // Use offset pagination like the admin page
  ...
  const selectItems = useMemo(
    () => (res: unknown) => {
      const result = res as TicketQueryResult;
      // Server-side filtering handles personalOnly, but keep as defensive fallback
      return (result.tickets || []).filter((ticket: TicketListItem) => !ticket.organizationId);
    },
    []
  );

  const selectHasMore = useMemo(
    () => (res: unknown) => {
      const result = res as TicketQueryResult;
  ...
  } = useOffsetPagination<TicketListItem, typeof baseArgs>({
  ...
  // Tickets are already filtered for personal tickets and date range in the query
  const filteredTickets = useMemo(() => ticketsPage || [], [ticketsPage]);
  ...
  const metrics = useMemo(() => {
    ...
    const highPriority = filteredTickets.filter((t) => t.priority === 'HIGH' && t.status !== 'CLOSED' && t.status !== 'RESOLVED').length;
  ...
  const handleCreateTicket = async (title: string, description: string, ticketPriority: TicketPriority) => {
    try {
      ...
      throw error; // Re-throw so CreateTicketDialog knows submission failed
    }
  };

  const handleQuickStatusChange = async (ticket: TicketListItem, newStatus: TicketStatus) => {
    try {
  ...
  // Filter assigned tickets to only show personal tickets
  const myTickets = useMemo(() => {
    if (myAssignedTicketsLoading || myAssignedTicketsError || !myAssignedTickets) {
      return [];
    }
    return (myAssignedTickets.tickets || []).filter((t) => !t.organizationId && (t.status === 'OPEN' || t.status === 'IN_PROGRESS'));
  ...
  return (
    <div className="space-y-6 font-admin-body pb-10">
      ...
      <PageHeader
        ...
        description="Manage and resolve personal support tickets (no organization)"
        icon={<Ticket className="h-5 w-5" />}
        breadcrumbs={[{ label: 'Super Admin', href: '/super-admin/overview' }, { label: 'Tickets' }]}
        actions={
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        {[
          ...
          >
            ...
            <p className={cn('text-2xl font-bold font-admin-heading', stat.color)}>{isLoading ? '—' : stat.value}</p>
          </motion.div>
        ))}
      </div>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="h-40">
                    <EmptyState
                      ...
                      description={
                        search || dateRange.dateFrom || dateRange.dateTo ? `No personal tickets match your filters` : 'No personal tickets found'
                      }
                      action={{
                        label: 'Create Ticket',
                ...
                filteredTickets.map((ticket, index) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell>
                        <Link href={`/super-admin/tickets/${ticket._id}`} className="block group-hover:text-primary transition-colors">
                          <div className="flex items-center gap-3">
                            <div
                              className={cn(
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-52">
                            <DropdownMenuItem asChild>
                              <Link href={`/super-admin/tickets/${ticket._id}`}>
