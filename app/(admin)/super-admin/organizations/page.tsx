...
export default function SuperAdminOrganizationsPage () {
  ...
  const userResults = useQuery(
    ...
    userSearch.trim().length >= 2
      ? { searchTerm: userSearch.trim(), limit: 5 }
      : 'skip'
  )
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="rounded border">
            ...
            <div>
              {organizations?.page?.map((org: { _id: string; name: string; slug: string; organizationType: 'PUBLIC' | 'PRIVATE' | 'SECRET'; memberCount: number }) => (
                <div key={org._id}>
                  <div className="grid grid-cols-12 items-center px-3 py-2 hover:bg-secondary">
                    ...
                    <div className="col-span-2 flex items-center justify-end gap-2 text-xs">
                      {inviteCodes[org._id] && (
                        <span className="rounded bg-muted px-2 py-0.5 font-mono">{inviteCodes[org._id]}</span>
                      )}
                      <Button size="sm" variant="outline" onClick={() => handleInvite(org._id)}>Invite</Button>
                      <Button size="sm" variant="outline" onClick={() => handleRename(org._id)}>Rename</Button>
                      <Button size="sm" variant="destructive" onClick={() => handleDelete(org._id)}>Delete</Button>
                      <Button size="sm" variant="outline" onClick={() => setOpenPermsForOrg((prev) => prev === org._id ? null : org._id)}>Permissions</Button>
                    </div>
                  </div>
                          ...
                          {userResults.map((u) => (
                            <div key={u._id} className="flex items-center justify-between rounded border bg-card px-3 py-2 text-xs">
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                <Button size="sm" variant="outline" onClick={() => assignOrganizationPermission({
                                  organizationId: org._id as unknown as Id<'organizations'>,
                                  userId: u._id as unknown as Id<'users'>,
                                  ...
                                  canUpdate: true,
                                  canDelete: false,
                                })}>Grant MANAGE_LOGS</Button>
                                <Button size="sm" variant="outline" onClick={() => revokeOrganizationPermission({
                                  organizationId: org._id as unknown as Id<'organizations'>,
                                  userId: u._id as unknown as Id<'users'>,
                                  permissionCode: 'MANAGE_LOGS',
                                })}>Revoke MANAGE_LOGS</Button>
                              </div>
                            </div>
                          ))}
                        </div>
