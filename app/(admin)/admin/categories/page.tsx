    ...
    <div>
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage category tree and settings</p>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <>
                {grouped.root.map((c) => (
                  <CategoryTableRow key={c._id} cat={c} childrenMap={grouped.children} />
                ))}
              </>
            )}
          </TableBody>
      ...
      {!loading && filtered.length === 0 && <div className="py-12 text-center text-sm text-muted-foreground">No categories found.</div>}
...
function CategoryTableRow({ cat, childrenMap, depth = 0 }: { cat: CategoryListItem; childrenMap: Map<string, CategoryListItem[]>; depth?: number }) {
      ...
      <React.Fragment key={category._id}>
        <TableRow className={currentDepth > 0 ? 'bg-muted/30' : ''}>
          <TableCell className="font-medium">
            <div className="flex items-center gap-2">
              {currentDepth > 0 && <span className="text-muted-foreground">{Array(currentDepth).fill('└─').join('')}</span>}
              {category.name}
            </div>
          </TableCell>
          <TableCell className="text-muted-foreground font-mono text-sm">{category.slug}</TableCell>
          <TableCell className="text-muted-foreground">{category.level}</TableCell>
          <TableCell>
