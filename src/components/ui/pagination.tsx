...
export function Pager ({ page, pageCount, onPageChange, className }: PagerProps) {
  ...
  return (
    <div className={['flex items-center gap-2', className || ''].join(' ').trim()}>
      <Button size="sm" variant="outline" onClick={() => onPageChange(current - 1)} disabled={current <= 1}>Prev</Button>
      {items.map(i => (
        <Button
          key={i}
          size="sm"
      ...
      <Button size="sm" variant="outline" onClick={() => onPageChange(current + 1)} disabled={current >= pages}>Next</Button>
    </div>
  )
}
