...
export function OrgMembersManager ({ organizationId }: Props) {
  ...
  const joinRequests = useQuery(
    ...
    activeTab === 'requests' ? { organizationId, status: 'PENDING' } : ('skip' as unknown as { organizationId: Id<'organizations'>, status: 'PENDING' })
  )
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <Dialog open={assignOpen} onOpenChange={setAssignOpen}>
                <DialogTrigger asChild>
                  <Button size="sm" disabled={selectedCount === 0}>Assign permissions ({selectedCount})</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <label className="flex items-center gap-1 text-sm"><input type="checkbox" className="h-4 w-4" checked={permCRUDE.canCreate} onChange={(e) => setPermCRUDE((s) => ({ ...s, canCreate: e.target.checked }))} /> Create</label>
                    <label className="flex items-center gap-1 text-sm"><input type="checkbox" className="h-4 w-4" checked={permCRUDE.canRead} onChange={(e) => setPermCRUDE((s) => ({ ...s, canRead: e.target.checked }))} /> Read</label>
                    <label className="flex items-center gap-1 text-sm"><input type="checkbox" className="h-4 w-4" checked={permCRUDE.canUpdate} onChange={(e) => setPermCRUDE((s) => ({ ...s, canUpdate: e.target.checked }))} /> Update</label>
                    <label className="flex items-center gap-1 text-sm"><input type="checkbox" className="h-4 w-4" checked={permCRUDE.canDelete} onChange={(e) => setPermCRUDE((s) => ({ ...s, canDelete: e.target.checked }))} /> Delete</label>
                  </div>
                  <DialogFooter>
                    <Button onClick={handleAssignPermissions} disabled={Object.values(selectedPermissionCodes).every((v) => !v)}>Assign</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
            ...
            {items.map((m) => (
              <div key={m._id} className="flex items-center justify-between gap-4 px-3 py-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" variant="destructive" onClick={() => handleRemove(m.userId)}>Remove</Button>
                </div>
              </div>
            ))}
            {items.length === 0 && (
          ...
          <div className="rounded border p-3">
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              ...
              <div className="flex items-end">
                <Button className="w-full" onClick={handleCreateInvite} disabled={!currentUser || creatingInvite}>{creatingInvite ? 'Creating...' : 'Create invite'}</Button>
              </div>
            </div>
          </div>
              ...
              {(inviteLinks || []).map((inv: InviteLink) => (
                <div key={inv._id} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">Uses: {inv.usedCount}/{inv.usageLimit || '∞'}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={() => navigator.clipboard.writeText(inv.code)}>Copy code</Button>
                    <Button size="sm" variant="destructive" onClick={() => deactivateInvite({ inviteLinkId: inv._id })}>Deactivate</Button>
                  </div>
                </div>
              ))}
              {(!inviteLinks || inviteLinks.length === 0) && (
                <div className="px-3 py-6 text-sm text-muted-foreground">No active invites.</div>
              )}
            </div>
          </div>
        </div>
              ...
              {joinRequests?.page?.map((req: { _id: Id<'organizationJoinRequests'>; userId: string; createdAt: number; note?: string }) => (
                <div key={req._id} className="flex items-center justify-between gap-3 px-3 py-2 text-sm">
                  <div className="min-w-0">
                    ...
                    {req.note && (
                      <div className="mt-1 text-xs text-muted-foreground line-clamp-2">{req.note}</div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={() => reviewJoinRequest({ organizationId, requestId: req._id, approve: false })}>Reject</Button>
                    <Button size="sm" onClick={() => reviewJoinRequest({ organizationId, requestId: req._id, approve: true })}>Approve</Button>
                  </div>
                </div>
              ))}
...
function MemberAvatar ({ imageUrl, firstName, lastName }: { imageUrl?: string; firstName?: string; lastName?: string }) {
  ...
  const url = useQuery(
    ...
    isKey(imageUrl) ? { key: imageUrl as string } : ('skip' as unknown as { key: string })
  )
