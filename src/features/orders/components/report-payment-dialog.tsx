...
const schema = z.object({
  amount: z
    ...
    .or(z.string().regex(/^\d+(?:[.,]\d+)?$/, 'Invalid amount'))
...
export function ReportPaymentDialog({
  ...
  const currentUser = useQuery(api.users.queries.index.getCurrentUser, userId ? { clerkId: userId } : ('skip' as unknown as { clerkId: string }));
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Submit your payment details for this order. An admin will verify it shortly.</DialogDescription>
        </DialogHeader>
