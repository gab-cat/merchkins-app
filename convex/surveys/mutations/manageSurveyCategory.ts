...
export const deleteSurveyCategoryHandler = async (
  ctx: MutationCtx,
  args: { categoryId: Id<"surveyCategories">; force?: boolean }
) => {
  const user = await requireAdmin(ctx);
  const existing = await ctx.db.get(args.categoryId);
  if (!existing) throw new Error("Survey category not found");
  ...
  if (args.force) {
    ...
    await logAction(ctx, "hard_delete_survey_category", "DATA_CHANGE", "HIGH", `Hard deleted survey category: ${existing.name}`, user._id, undefined, { categoryId: args.categoryId });
    return { success: true };
  }
  ...
  await logAction(ctx, "delete_survey_category", "DATA_CHANGE", "MEDIUM", `Deleted survey category: ${existing.name}` , user._id, undefined, { categoryId: args.categoryId });
  return { success: true };
};
...
export const restoreSurveyCategoryHandler = async (
  ctx: MutationCtx,
  args: { categoryId: Id<"surveyCategories"> }
) => {
  const user = await requireAdmin(ctx);
  const existing = await ctx.db.get(args.categoryId);
  if (!existing) throw new Error("Survey category not found");
  ...
  await logAction(ctx, "restore_survey_category", "DATA_CHANGE", "MEDIUM", `Restored survey category: ${existing.name}`, user._id, undefined, { categoryId: args.categoryId });
  return { success: true };
};
