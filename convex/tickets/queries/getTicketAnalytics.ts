...
export const getTicketAnalyticsHandler = async (ctx: QueryCtx, args: { forUserOnly?: boolean }) => {
  ...
  const rows = privileged && !args.forUserOnly ? await ctx.db.query('tickets').collect() : [...creatorTickets, ...assigneeTickets];
