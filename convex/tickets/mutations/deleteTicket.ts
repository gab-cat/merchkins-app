...
export const deleteTicketHandler = async (
  ctx: MutationCtx,
  args: { ticketId: Id<"tickets">; force?: boolean }
) => {
  ...
  if (args.force && user.isAdmin) {
    ...
    await logAction(ctx, "hard_delete_ticket", "DATA_CHANGE", "HIGH", `Hard deleted ticket ${args.ticketId}`, user._id, undefined, { ticketId: args.ticketId });
  } else {
    ...
    await logAction(ctx, "archive_ticket", "DATA_CHANGE", "LOW", `Archived ticket ${args.ticketId}`, user._id, undefined, { ticketId: args.ticketId });
  }
