...
export const updateTicketHandler = async (
  ...
  for (const change of updatesToLog) {
    const updateId = await ctx.db.insert('ticketUpdates', {
      ...
      ticketInfo: {
        title: (updateData.title as string) || ticket.title,
        priority: (updateData.priority as string) || ticket.priority,
        category: (updateData.category as string) || ticket.category,
      },
  ...
  await logAction(ctx, 'update_ticket', 'DATA_CHANGE', 'LOW', `Updated ticket ${args.ticketId}`, user._id, undefined, {
