...
export const assignTicketHandler = async (
  ctx: MutationCtx,
  args: { ticketId: Id<"tickets">; assigneeId: Id<"users"> }
) => {
  ...
  const updateId = await ctx.db.insert("ticketUpdates", {
    ...
    previousValue: ticket.assigneeInfo ? `${ticket.assigneeInfo.firstName || ""} ${ticket.assigneeInfo.lastName || ""}`.trim() || ticket.assigneeInfo.email : undefined,
  ...
  await logAction(
    ...
    "DATA_CHANGE",
    "LOW",
    `Assigned ticket ${args.ticketId} to ${assignee.email}`,
    user._id,
    undefined,
    { ticketId: args.ticketId, assigneeId: args.assigneeId }
  );
