...
export const addVariantHandler = async (
  ...
  const existingVariant = product.variants.find((v) => v.variantName.toLowerCase() === args.variantName.toLowerCase());
...
export const updateVariantHandler = async (
  ...
  if (args.variantName !== undefined) {
    ...
    const nameConflict = product.variants.find(
      (v, index) => index !== variantIndex && v.variantName.toLowerCase() === args.variantName!.toLowerCase()
    );
