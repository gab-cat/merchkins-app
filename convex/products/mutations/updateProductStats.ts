...
import { validateProductExists, validateNonNegativeNumber } from '../../helpers';
...
export const updateProductStatsHandler = async (
  ...
  if (args.variantUpdates && args.variantUpdates.length > 0) {
    const updatedVariants = product.variants.map((variant) => {
      const variantUpdate = args.variantUpdates!.find((update) => update.variantId === variant.variantId);
