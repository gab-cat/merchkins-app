...
export const getProductsHandler = async (
  ...
  if (!args.organizationId && args.categoryId) {
    ...
    for (const cid of categoryIdsToInclude) {
      const baseQuery = ctx.db.query('products').withIndex('by_category', (q) => q.eq('categoryId', cid));
    ...
    if (args.organizationId && args.categoryId) {
      ...
      query = ctx.db.query('products').withIndex('by_creator', (q) => q.eq('postedById', args.postedById!));
    } else if (args.inventoryType) {
      query = ctx.db.query('products').withIndex('by_inventory_type', (q) => q.eq('inventoryType', args.inventoryType!));
    } else if (args.isBestPrice !== undefined) {
      query = ctx.db.query('products').withIndex('by_best_price', (q) => q.eq('isBestPrice', args.isBestPrice!));
