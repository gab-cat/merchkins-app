...
export const getProductAnalyticsHandler = async (
  ...
  if (args.productId) {
    ...
    return {
      ...
      totalRevenue: product.variants.reduce((sum, variant) => sum + variant.price * variant.orderCount, 0),
  ...
  if (args.timeframe && args.timeframe !== 'all') {
    ...
    query = query.filter((q) => q.and(q.eq(q.field('isDeleted'), false), q.gte(q.field('createdAt'), timeThreshold)));
  } else {
  ...
  const analytics = products.reduce(
    (acc, product) => {
      const productRevenue = product.variants.reduce((sum, variant) => sum + variant.price * variant.orderCount, 0);
  ...
  const averageRating = analytics.reviewsCount > 0 ? analytics.ratingSum / analytics.reviewsCount : 0;

  const averageOrdersPerProduct = analytics.totalProducts > 0 ? analytics.totalOrders / analytics.totalProducts : 0;

  const averageRevenuePerProduct = analytics.totalProducts > 0 ? analytics.totalRevenue / analytics.totalProducts : 0;

  const averageRevenuePerOrder = analytics.totalOrders > 0 ? analytics.totalRevenue / analytics.totalOrders : 0;

  // Get top performing products
  const topProducts = products
    .sort((a, b) => {
      const aRevenue = a.variants.reduce((sum, variant) => sum + variant.price * variant.orderCount, 0);
      const bRevenue = b.variants.reduce((sum, variant) => sum + variant.price * variant.orderCount, 0);
      return bRevenue - aRevenue;
    })
    .slice(0, 10)
    .map((product) => {
      const revenue = product.variants.reduce((sum, variant) => sum + variant.price * variant.orderCount, 0);
      return {
  ...
  const categoryBreakdown = args.categoryId
    ? []
    : products.reduce(
        (acc, product) => {
