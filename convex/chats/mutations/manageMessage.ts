...
export const editMessageHandler = async (ctx: MutationCtx, args: { messageId: Id<'chatMessages'>; content: string }) => {
  ...
  const editHistory = [...(message.editHistory || []), { content: message.content, editedAt: now }];
  ...
  await logAction(ctx, 'edit_chat_message', 'DATA_CHANGE', 'LOW', `Edited message ${args.messageId}`, currentUser._id, undefined, {
...
export const deleteMessageHandler = async (ctx: MutationCtx, args: { messageId: Id<'chatMessages'> }) => {
  const currentUser = await requireAuthentication(ctx);
  ...
  await logAction(ctx, 'delete_chat_message', 'DATA_CHANGE', 'LOW', `Deleted message ${args.messageId}`, currentUser._id, undefined, {
    messageId: args.messageId,
  });
...
export const togglePinMessageHandler = async (ctx: MutationCtx, args: { messageId: Id<'chatMessages'>; isPinned: boolean }) => {
  const currentUser = await requireAuthentication(ctx);
...
export const reactToMessageHandler = async (ctx: MutationCtx, args: { messageId: Id<'chatMessages'>; emoji: string }) => {
  const currentUser = await requireAuthentication(ctx);
  const now = Date.now();
  const message = await ctx.db.get(args.messageId);
  if (!message || message.isDeleted) {
...
export const markRoomReadHandler = async (ctx: MutationCtx, args: { chatRoomId: Id<'chatRooms'> }) => {
  const currentUser = await requireAuthentication(ctx);
  const now = Date.now();
  ...
  const updated = state.unreadCounts.map((u) => (u.userId === currentUser._id ? { ...u, count: 0, lastReadAt: now } : u));
