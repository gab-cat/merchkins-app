...
export const generatePayoutInvoicesHandler = async (
  ...
  for (const org of organizations) {
    ...
    // Handle negative net amount (carry over logic)
    let netAmount = 0;
    let carryOverAmount = 0;

    if (adjustedNetAmount < 0) {
      // Net amount is negative, so payout is 0
      netAmount = 0;
      // The entire negative amount is carried over
      carryOverAmount = adjustedNetAmount;

      console.log(`[generatePayoutInvoices] ${org.name}: Negative balance ${adjustedNetAmount} will be carried over`);
    } else {
    ...
    // Determine initial status - if net amount is 0, it's considered PAID immediately
    const initialStatus = netAmount === 0 ? 'PAID' : 'PENDING';
    const statusReason = netAmount === 0 ? 'Invoice amount is 0, marked as PAID' : 'Invoice generated by system';
    ...
    // If there is a carry-over amount, create a new adjustment for the next period
    if (carryOverAmount < 0) {
      await ctx.db.insert('payoutAdjustments', {
        organizationId: org._id,
        // No orderId for carry-over as it's an aggregate adjustment
        orderId: undefined,
        originalInvoiceId: invoiceId, // Linked to the invoice that generated this carry-over
        type: 'CARRY_OVER',
        amount: carryOverAmount,
        reason: `Negative balance carried forward from Invoice #${invoiceNumber}`,
        status: 'PENDING',
        createdAt: now,
      });

      console.log(`[generatePayoutInvoices] Created carry-over adjustment of ${carryOverAmount} for ${org.name}`);
    }
  ...
  return {
    success: true,
    invoicesCreated, // Return the array of IDs
    periodStart: args.periodStart,
    periodEnd: args.periodEnd,
  };
};
