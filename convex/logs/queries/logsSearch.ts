...
export const searchLogsHandler = async (ctx: QueryCtx, args: { searchTerm: string; organizationId?: Id<'organizations'>; limit?: number }) => {
  ...
  const filtered = rows.filter((l) => {
    ...
    const inCreator =
      f(l.creatorInfo?.email).includes(term) || f(l.creatorInfo?.firstName).includes(term) || f(l.creatorInfo?.lastName).includes(term);
