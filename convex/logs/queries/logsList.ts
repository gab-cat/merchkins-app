...
export const getLogsHandler = async (
  ...
  if (args.organizationId) {
    ...
    if (args.logType) {
      baseQuery = ctx.db
        .query('logs')
        .withIndex('by_organization_type', (q) => q.eq('organizationId', args.organizationId!).eq('logType', args.logType as any));
    } else {
      baseQuery = ctx.db.query('logs').withIndex('by_organization', (q) => q.eq('organizationId', args.organizationId!));
    }
  } else {
