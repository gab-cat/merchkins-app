...
export const getPopularCategoriesArgs = {
  ...
  metric: v.optional(v.union(v.literal('products'), v.literal('orders'), v.literal('revenue'))),
...
export const getPopularCategoriesHandler = async (
  ...
  if (args.organizationId) {
    query = ctx.db.query('categories').withIndex('by_organization_active', (q) => q.eq('organizationId', args.organizationId!).eq('isActive', true));
  } else {
    query = ctx.db.query('categories').withIndex('by_active', (q) => q.eq('isActive', true));
  }

  let categories = await query.filter((q) => q.eq(q.field('isDeleted'), false)).collect();
