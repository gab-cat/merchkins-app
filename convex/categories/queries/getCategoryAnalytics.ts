...
export const getCategoryAnalyticsHandler = async (
  ...
  const categories = await query.filter((q) => q.eq(q.field('isDeleted'), false)).collect();
  ...
  const analytics = {
    ...
    averageProductsPerCategory:
      categories.length > 0 ? Math.round((categories.reduce((sum, c) => sum + c.productCount, 0) / categories.length) * 100) / 100 : 0,
  };
