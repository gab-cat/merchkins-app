...
import { requireAuthentication, validateStringLength, validatePositiveNumber, logAction, requireOrganizationPermission } from '../../helpers';
...
export const updatePaymentHandler = async (
      ...
      const dup = await ctx.db
        ...
        .filter((q) =>
          q.and(
            ...
            existing.organizationId ? q.eq(q.field('organizationId'), existing.organizationId) : q.eq(q.field('orderId'), existing.orderId)
          )
        )
        .first();
  ...
  if (args.transactionId !== undefined) {
    if (args.transactionId && args.transactionId !== existing.transactionId) {
      const dupTxn = await ctx.db
        ...
        .filter((q) => q.and(q.eq(q.field('isDeleted'), false), q.eq(q.field('transactionId'), args.transactionId!)))
        .first();
  ...
  await ctx.runMutation(internal.payments.mutations.index.updatePaymentStats, {
    ...
    actorName: `${currentUser.firstName ?? ''} ${currentUser.lastName ?? ''}`.trim() || currentUser.email,
  });
