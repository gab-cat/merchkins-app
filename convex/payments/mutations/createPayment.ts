...
export const createPaymentHandler = async (
  ...
  if (args.organizationId) {
    ...
    await requireOrganizationPermission(ctx, args.organizationId, 'MANAGE_PAYMENTS', 'create');
  } else {
  ...
  const existingWithRef = await ctx.db
    ...
    .filter((q) =>
      q.and(
        ...
        args.organizationId ? q.eq(q.field('organizationId'), args.organizationId) : q.eq(q.field('orderId'), args.orderId)
      )
    )
    .first();
  if (existingWithRef) {
