...
export const deletePermissionHandler = async (
  ...
  if (!args.force) {
    ...
    const usersWithPermission = await ctx.db
      .query('users')
      .filter((q) => q.and(q.eq(q.field('isDeleted'), false), q.neq(q.field('permissions'), undefined)))
      .collect();

    const usersUsingPermission = usersWithPermission.filter((user) => user.permissions?.some((p) => p.permissionCode === permission.code));
    ...
    const membersUsingPermission = orgMembersWithPermission.filter((member) => member.permissions?.some((p) => p.permissionCode === permission.code));

    if (membersUsingPermission.length > 0) {
      throw new Error(
        `Cannot delete permission: ${membersUsingPermission.length} organization members are using this permission. Use force=true to delete anyway.`
      );
    }
  }
